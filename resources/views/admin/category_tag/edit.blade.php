                ...
                <div class="span12">
                    ...
                    <div class="block-fluid">
                        <div class="row-form clearfix">
                            ...
                            <div class="span9">
                                <input type="text" value="{{ $tag->title }}" class="validate[required]" id="title" name="title"/>
                            </div>
                        </div>
                                ...
                                <select name="select" class="validate[required]" id="category_id">
                                    ...
                                    @if(count($categorys))
                                        @foreach($categorys as $category)
                                            <option value="{{$category->id}}" {{ $tag->category_id==$category->id?'selected':'' }}>{{$category->title}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
        ...
        $(document).ready(function() {
            $('#submit').click(function(e) {
                ...
                if ($("#validation").validationEngine('validate')) {
                    var url = "{{route('category_tags.update',['id'=>$tag->id])}}";
                    var title = $("#title").val();
                    var _method = 'PUT';
                    var category_id = $("#category_id").val();

                    $.post(url,
                        {title:title,_method:_method,category_id:category_id},
                        function(data){
                            cTip(data);
                        }, "json");
                }
