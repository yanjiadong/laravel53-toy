...
class OrderController extends BaseController
{
    ...
    public function submit_order_new(Request $request)
    {
        ...
        if($user->wechat_openid == 'o2xFAwxEAUOs6wDaUt07BH2C8Fbs' || $user->wechat_openid == 'o2xFAw0CyeI2Z5mSUXeWg13RCvz4' || $user->wechat_openid == 'o2xFAw7K6g1yHtZ-MvYFX2gYRzpI' || $user->wechat_openid  == 'o2xFAw1y8Qlz5jQvcMwEzWXHB3bg' || $user->wechat_openid == 'o2xFAw_Nkx7In3f84SDJ0o2pp130')
        {
        ...
        if(!empty($list))
        {
            foreach ($list as &$v)
            {
                ...
                elseif($v['status']==Order::STATUS_DOING_STR)
                {
                    ...
                    {
                        $v['days2'] = floor((strtotime($v['end_time']) - $this->time)/86400);
                    }
                }
    ...
    public function order_detail(Request $request)
    {
        ...
        $express_info = ExpressInfo::where('nu',$info->express_no)->where('type',0)->orderBy('id','desc')->first();
        if(!empty($express_info) && !empty($express_info->content_list))
        {
            $content = json_decode($express_info->content_list,true);
            $logistics['time'] = $content[0]['time'];
            $logistics['context'] = $content[0]['context'];
        }
        ...
        if($info['status'] == Order::STATUS_DOING_STR)
        {
            ...
            {
                $info['days2'] = floor((strtotime($info['end_time']) - $this->time)/86400);
            }
        }
    ...
    public function logistics_detail(Request $request)
    {
        ...
        $express_info = ExpressInfo::where('nu',$nu)->where('type',0)->orderBy('id','desc')->first();

        $logistics = array();

        if(!empty($express_info) && !empty($express_info->content_list))
        {
            $logistics = json_decode($express_info->content_list,true);
        }
